...
app = Flask(__name__)
# Habilitamos CORS para que la web (Frontend) hable con este servidor (Backend)
CORS(app)

app.config['UPLOAD_FOLDER'] = '/tmp/uploads'
os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)

# API KEYS DESDE VARIABLES DE ENTORNO (Configurar en el panel de Render)
...
def ai_query():
    if not GEMINI_API_KEY:
        return jsonify({'error': 'Clave de Gemini no configurada en Render'}), 500
    
    data = request.json
    prompt = data.get('prompt')
    system_instruction = data.get('systemInstruction', 'Eres un experto en prospección de ventas en Argentina.')
    ...
    try:
        ...
        if 'candidates' in result:
            ...
            return jsonify({'error': 'Error en respuesta de Gemini'}), 500
    except Exception as e:
        return jsonify({'error': str(e)}), 500
