...
def search_places():
    ...
    try:
        ...
        if not all_results:
            logger.warning(f"No se encontraron resultados para la zona: {zona}")
            return jsonify({'success': True, 'leads': [], 'total': 0}), 200
        
        # Limitar a 40 resultados para mejor performance
        results = all_results[:40]
        logger.info(f"Procesando {len(results)} resultados (límite: 40 para mejor performance)...")
