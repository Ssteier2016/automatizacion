...
def buscar_emails_con_ia(texto_pagina, url):
    """Utiliza IA para encontrar emails en el texto de la página."""
    ...
    try:
        ...
        logger.debug(f"Error en IA para búsqueda de emails: {e}")
    
    return ""

def buscar_redes_con_ia(texto_pagina, url):
    """Utiliza IA para encontrar redes sociales."""
    if not GEMINI_API_KEY:
    ...
    try:
        ...
        logger.debug(f"Error en IA para búsqueda de redes: {e}")
    
    return {"facebook": "", "instagram": ""}

def scraping_profundo_contacto(url_base, exhaustivo=False):
    """
    Busca emails y redes sociales priorizando la búsqueda de emails.
